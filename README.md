...
The compiler pipeline (tokenizer → parser → semantic analysis → bytecode generation) works end-to-end for a core subset of the language: functions, variables, arithmetic, comparisons, `and`/`or`, assignment, `if`/`elseif`/`else`, `while`, function calls, and `return`. There's no VM/JIT yet, so nothing runs -- compiled output is verified by disassembling and hand-tracing the generated bytecode.

The project has many holes still:

- Lambdas and function-as-value are parsed but not type-checked or compiled
- Structs, pointers, and `new`-based allocation are parsed but not type-checked or compiled
- Lists unimplemented
- No runtime (VM/JIT) implementation yet
